...
var init_lib = __esm({
  "node_modules/@ethersproject/logger/lib.esm/index.js"() {
    ...
    Logger = class _Logger {
      ...
      static from(version15) {
        return new _Logger(version15);
      }
    };
...
var init_version5 = __esm({
  "node_modules/@ethersproject/abstract-provider/lib.esm/_version.js"() {
    version5 = "abstract-provider/5.7.0";
  }
});
...
var __awaiter3, logger10, padding, NegativeOne3, Zero3, One2, MaxUint2562, hexTrue, hexFalse, domainFieldTypes, domainFieldNames, domainChecks, TypedDataEncoder;
...
var __awaiter4, logger11, allowedTransactionKeys, forwardErrors, Signer, VoidSigner;
var init_lib14 = __esm({
  "node_modules/@ethersproject/abstract-signer/lib.esm/index.js"() {
    ...
    Signer = class _Signer {
      ...
      constructor() {
        logger11.checkAbstract(new.target, _Signer);
      ...
      checkTransaction(transaction) {
        for (const key2 in transaction) {
          if (allowedTransactionKeys.indexOf(key2) === -1) {
            logger11.throwArgumentError("invalid transaction key: " + key2, "transaction", transaction);
          }
        }
      ...
      populateTransaction(transaction) {
        return __awaiter4(this, void 0, void 0, function* () {
          ...
          if (tx.gasPrice != null && (tx.type === 2 || hasEip1559)) {
            logger11.throwArgumentError("eip-1559 transaction do not support gasPrice", "transaction", transaction);
          } else if ((tx.type === 0 || tx.type === 1) && hasEip1559) {
            logger11.throwArgumentError("pre-eip-1559 transaction do not support maxFeePerGas/maxPriorityFeePerGas", "transaction", transaction);
          }
          if ((tx.type === 2 || tx.type == null) && (tx.maxFeePerGas != null && tx.maxPriorityFeePerGas != null)) {
            ...
            if (tx.type == null) {
              if (feeData.maxFeePerGas != null && feeData.maxPriorityFeePerGas != null) {
                ...
                if (hasEip1559) {
                  logger11.throwError("network does not support EIP-1559", Logger.errors.UNSUPPORTED_OPERATION, {
                ...
                logger11.throwError("failed to get consistent fee data", Logger.errors.UNSUPPORTED_OPERATION, {
          ...
          if (tx.gasLimit == null) {
            tx.gasLimit = this.estimateGas(tx).catch((error) => {
              ...
              return logger11.throwError("cannot estimate gas; transaction may fail or may require manual gas limit", Logger.errors.UNPREDICTABLE_GAS_LIMIT, {
                error,
                tx
              });
            });
          ...
          if (tx.chainId == null) {
            ...
            tx.chainId = Promise.all([
              ...
              if (results[1] !== 0 && results[0] !== results[1]) {
                logger11.throwArgumentError("chainId address mismatch", "transaction", transaction);
              }
              return results[0];
            });
          }
      ...
      _checkProvider(operation) {
        if (!this.provider) {
          logger11.throwError("missing provider", Logger.errors.UNSUPPORTED_OPERATION, {
    ...
    VoidSigner = class _VoidSigner extends Signer {
      ...
      _fail(message, operation) {
        return Promise.resolve().then(() => {
          logger11.throwError(message, Logger.errors.UNSUPPORTED_OPERATION, { operation });
        });
      }
      signMessage(message) {
...
function computePublicKey(key2, compressed) {
  ...
  return logger12.throwArgumentError("invalid public or private key", "key", "[REDACTED]");
}
var logger12, _curve, SigningKey;
    ...
    SigningKey = class {
      constructor(privateKey) {
        ...
        if (hexDataLength(this.privateKey) !== 32) {
          logger12.throwArgumentError("invalid private key", "privateKey", "[[ REDACTED ]]");
        }
      ...
      signDigest(digest) {
        ...
        if (digestBytes.length !== 32) {
          logger12.throwArgumentError("bad digest length", "digest", digest);
        }
...
function formatNumber(value, name) {
  ...
  if (result.length > 32) {
    logger13.throwArgumentError("invalid length for " + name, "transaction:" + name, value);
  }
  return result;
}
  ...
  return {
    ...
    storageKeys: (storageKeys || []).map((storageKey, index) => {
      if (hexDataLength(storageKey) !== 32) {
        logger13.throwArgumentError("invalid access list storageKey", `accessList[${addr}:${index}]`, storageKey);
      }
  ...
  if (Array.isArray(value)) {
    return value.map((set, index) => {
      if (Array.isArray(set)) {
        if (set.length > 2) {
          logger13.throwArgumentError("access list expected to be [ address, storageKeys[] ]", `value[${index}]`, set);
        }
...
function _serializeEip1559(transaction, signature2) {
  if (transaction.gasPrice != null) {
    ...
    if (!gasPrice.eq(maxFeePerGas)) {
      logger13.throwArgumentError("mismatch EIP-1559 gasPrice != maxFeePerGas", "tx", {
        gasPrice,
        maxFeePerGas
      });
    }
...
function _serialize(transaction, signature2) {
  ...
  transactionFields.forEach(function(fieldInfo) {
    ...
    if (fieldInfo.length && value.length !== fieldInfo.length && value.length > 0) {
      logger13.throwArgumentError("invalid length for " + fieldInfo.name, "transaction:" + fieldInfo.name, value);
    }
  ...
  if (transaction.chainId != null) {
    ...
    if (typeof chainId !== "number") {
      logger13.throwArgumentError("invalid transaction.chainId", "transaction", transaction);
    }
  ...
  if (chainId !== 0) {
    ...
    v += chainId * 2 + 8;
    if (sig.v > 28 && sig.v !== v) {
      logger13.throwArgumentError("transaction.chainId/signature.v mismatch", "signature", signature2);
    }
  } else if (sig.v !== v) {
    logger13.throwArgumentError("transaction.chainId/signature.v mismatch", "signature", signature2);
  }
...
function serialize(transaction, signature2) {
  if (transaction.type == null || transaction.type === 0) {
    if (transaction.accessList != null) {
      logger13.throwArgumentError("untyped transactions do not support accessList; include type: 1", "transaction", transaction);
    }
  ...
  return logger13.throwError(`unsupported transaction type: ${transaction.type}`, Logger.errors.UNSUPPORTED_OPERATION, {
...
function _parseEipSignature(tx, fields, serialize2) {
  try {
    ...
    logger13.throwArgumentError("invalid v for transaction type: 1", "v", fields[0]);
  }
...
function _parseEip1559(payload) {
  ...
  if (transaction.length !== 9 && transaction.length !== 12) {
    logger13.throwArgumentError("invalid component count for transaction type: 2", "payload", hexlify(payload));
  }
...
function _parseEip2930(payload) {
  ...
  if (transaction.length !== 8 && transaction.length !== 11) {
    logger13.throwArgumentError("invalid component count for transaction type: 1", "payload", hexlify(payload));
  }
  const tx = {
    type: 1,
    chainId: handleNumber(transaction[0]).toNumber(),
...
function _parse(rawTransaction) {
  ...
  if (transaction.length !== 9 && transaction.length !== 6) {
    logger13.throwArgumentError("invalid raw transaction", "rawTransaction", rawTransaction);
  }
  const tx = {
...
function parse(rawTransaction) {
  ...
  return logger13.throwError(`unsupported transaction type: ${payload[0]}`, Logger.errors.UNSUPPORTED_OPERATION, {
    operation: "parseTransaction",
    transactionType: payload[0]
  });
}
var logger13, TransactionTypes, transactionFields, allowedTransactionKeys2;
...
function computeHmac(algorithm, key2, data) {
  if (!SupportedAlgorithm[algorithm]) {
    logger14.throwError("unsupported algorithm " + algorithm, Logger.errors.UNSUPPORTED_OPERATION, {
      operation: "hmac",
      algorithm
    });
  }
...
js-sha3/src/sha3.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-GS2N2WBR.js.map
